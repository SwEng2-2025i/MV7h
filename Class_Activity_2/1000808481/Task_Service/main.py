...
def create_task():
    ...
    try:
        ...
        return (
            jsonify({"error": f"Error de conexión al verificar usuario: {str(e)}"}),
            500,
        )
...
def get_tasks():
    ...
    return jsonify(
        [{"id": t.id, "title": t.title, "user_id": t.user_id} for t in tasks]
    )


# Added endpoint to delete all tasks for a specific user
@service_b.route("/tasks/user/<int:user_id>", methods=["DELETE"])
def delete_tasks_by_user(user_id):
    try:
        tasks = Task.query.filter_by(user_id=user_id).all()
        if not tasks:
            return jsonify({"message": f"No tasks found for user {user_id}"}), 200

        for task in tasks:
            db.session.delete(task)
        db.session.commit()

        return jsonify({"message": f"All tasks for user {user_id} deleted"}), 200
