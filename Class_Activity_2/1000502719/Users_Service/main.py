...
@service_a.route('/cleanTestingUser',methods=["DELETE"])
def clean_test_user():
    data = request.get_json()
    user_id = data.get("id")

    if user_id is None:
        return jsonify({"error": "Falta el ID del user"}), 400

    user = db.session.get(User, user_id) 

    if user is None:
        return jsonify({"error": f"No se encontró un user con ID {user_id}"}), 404
